...
function init() {
  ...
  singleMap && ymaps.ready(function () {
    ...
    {// hintContent: 'ЖК',
      ...
      iconImageHref: backend_data.theme_url + '/img/icons/home.svg',
      iconImageSize: [46, 46],
...
jQuery( document ).ready(function($) {
  ...
  function apply_filters(e){
    let formdata = new FormData($('#page-filter')[0]);
    formdata.append('action','filter_action');
    ...
    .done(function( html ) {
      if ( $.trim(html) == 'no_data' ) {
        $('.show-more').hide();
      } else {
        $('.show-more').show();
        if (1 == page) {
          $('.page-loop').empty().append(html);
        } else {
          $('.page-loop').append(html);
        }
        $('.favorites-link').off("click").click(set_like_handler);
      }
    });
  }
  ...
  $('.favorites-link').click(set_like_handler);

  function set_like_handler(e){
    e.preventDefault();
    if ($(this).hasClass('favorites-link__add')) {
      $(this).removeClass('favorites-link__add');
      $(this).addClass('favorites-link__delete');
      let property_id = $(this).closest('[data-property_id]').data('property_id')
      if (!likes.includes(property_id)) {
        likes.push(property_id);
      }
    } else {
      $(this).removeClass('favorites-link__delete');
      $(this).addClass('favorites-link__add');
      likes.remove($(this).closest('[data-property_id]').data('property_id'));
    }
    setCookie('likes',JSON.stringify(likes),{path:'/'});
  }
